...
=============================================================================
 ...
 用法:
   ...
   可选开关:     -NoPause                      结束时不暂停(脚本化调用用)
                 -InstallDir <安装目录>         手动指定安装位置
...
# 兜底: 任何未捕获的终止错误也先显示出来再暂停, 避免窗口一闪而过
trap {
    Say ''
    Say ('发生错误: ' + $_.Exception.Message) 'Red'
    if($_.InvocationInfo){ Say ('  位置: ' + $_.InvocationInfo.PositionMessage.Trim()) 'DarkGray' }
    Pause-End
    exit 1
}
...
function Pause-End {
    # 防止双击/右键运行时窗口一闪而过; 加 -NoPause 可跳过
    if($NoPause){ return }
    if($Host.Name -eq 'Windows PowerShell ISE Host'){ return }
    Say ''
    try { [void](Read-Host '按回车键关闭窗口') } catch { }
}
...
if(-not (Test-Admin)){
    Say '需要管理员权限, 正在请求提权 (请在弹窗中点击[是])...' 'Yellow'
    ...
    foreach($k in @($PSBoundParameters.Keys)){
        ...
        if($v -is [switch]){
            ...
            $argList += ('-' + $k)
            foreach($it in $v){ $argList += [string]$it }
        } else {
            $argList += @(('-' + $k), [string]$v)
        }
    }
    ...
    if($started){
        Say '已在新窗口以管理员身份启动, 本窗口关闭。' 'Green'
    } else {
        Say '提权被取消或失败。' 'Red'
        Say ('  可手动: 右键 ' + $Script:SelfPath + ' -> 以管理员身份运行') 'Yellow'
        Say '  或直接双击同目录下的 .bat 启动器。' 'Yellow'
    }
    Pause-End
    exit
}
...
elseif($Remove.Count -gt 0){
    ...
    if($ans -eq 'none' -or $ans -eq ''){ Say '已取消' 'Yellow'; Pause-End; exit 0 }
    ...
    if($sel.Count -eq 0){ Say '未选择任何项目, 退出' 'Yellow'; Pause-End; exit 0 }
    Say ''
    Say ('将执行: ' + ($sel -join ', ')) 'Yellow'
    $go = Read-Host '确认执行? (y/N)'
    if($go.Trim().ToLower() -ne 'y'){ Say '已取消' 'Yellow'; Pause-End; exit 0 }
}
